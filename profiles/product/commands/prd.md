...
Extract `{{EPIC_KEY}}` from Stage 2 output (line matching `EPIC: MOBNEW-\d+`).
...
When {{TICKET_SYSTEM}} is jira:
  ...
  Capture each returned MOBNEW-XXX key. Link blocked stories to blockers:
  ...
  Print created-ticket summary:
    ...
    |------------|------------------------------|----------|------|
    | MOBNEW-XXX | [story one-line summary]     | High     | [N]  |
...
- `{{EPIC_KEY}}` = JIRA epic key extracted from Stage 2 output (line "EPIC: MOBNEW-XXX"), else empty
